...
- `src/imageProcessor.ts`: 件数・容量・画素を制限し、画像をメタデータなしの PNG へ正規化して、全件成功した場合だけ URL map を返す。
...
- `src/consent.ts`: ワークスペースフォルダー、送信先、同意文面の版ごとに同意と撤回を管理する。
...
5. 外部送信は既定で無効にし、同意後も正規化 PNG と匿名名だけを送る。URL、ハッシュ、原文の参照先は実行中メモリだけに置く。

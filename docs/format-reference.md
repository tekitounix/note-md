...
記事には h1 がちょうど一つ必要です。ATX (`#`) と Setext (`====`) のどちらもタイトルとして認識します。
...
- GIF / HEIC / AVIF と未知形式は安全な PNG に自動変換しないため、事前に PNG または JPEG へ変換します
- 本文コピー時にはアップロード済み URL に自動で差し替えられます
- `https:` / `data:` の外部画像はアップロード・到達性・CORS 検証の対象外です。公開前に note 側で表示を確認します
- note は表示幅 620px に縮小しますが、拡大時は原寸（最大 4000px）で表示されます
- ツールは画像を自動拡大しません。高密度表示も重視する場合は、横幅 1240px 以上の元画像を用意します
